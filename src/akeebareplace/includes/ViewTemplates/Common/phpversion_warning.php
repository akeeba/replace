...
(
	defined('WPINC') || defined('APATH_BASE')
	|| defined('AKEEBA_COMMON_WRONGPHP') || defined('KICKSTART')
) or die;
...
if (!function_exists('akeeba_common_phpversion_warning'))
{
	 ...
	 * @throws  Exception
	 */
	function akeeba_common_phpversion_warning(array $config = [])
	{
		// Format: version => [maintenance_date, eol_date]
		$phpDates = [
			// PHP versions up to and including 5.5 only ever had an End of Life date.
			...
			// PHP versions 5.6 and later have an End of Support and End of Life date.
			...
			'8.0' => ['2022-11-26 00:00:00', '2023-11-26 00:00:00'],
			'8.1' => ['2023-11-25 00:00:00', '2024-11-25 00:00:00'],
			'8.2' => ['2024-11-24 00:00:00', '2025-11-24 00:00:00'],
			'8.3' => ['2025-12-08 00:00:00', '2026-12-08 00:00:00'],
		];
		 ...
		 *
		 * Older PHP versions don't even have support for DateTime, so we need these defaults to prevent this warning
		 * script from bringing the site down with an error.
		 */
		$isEol      = true;
		$isAncient  = true;
		$isCurrent  = false;
		...
		if ($isTooNew): ?>
			...
			<div class="<?= $class_priority_medium ?>">
				<h3>PHP version <?= $shortVersion ?> is newer than this software supports</h3>

				<p>
					Your site is currently using PHP <?= $longVersion ?>. This version of PHP was released after your currently installed version of <?= $softwareName ?> was released. As a result, we cannot guarantee that <?= $softwareName ?> will work correctly on your site.
				</p>
				<p>
					Please check for an updated version of <?= $softwareName ?>. Kindly note that it usually takes us a few days to weeks after the initial (X.Y<strong>.0</strong>) PHP version release before we publish a version of our software which supports it.
				</p>
			</div>
		<?php
		elseif ($isAncient): ?>
		...
		elseif ($isEol):
			...
			<div class="<?= $class_priority_medium ?>">
				<h3>Outdated PHP version <?= $shortVersion ?></h3>

				<p>
					Your site is currently using PHP <?= $longVersion ?>. This version of PHP has recently become <a href="https://php.net/eol.php" target="_blank">End-of-Life since <?= $eolDateFormatted ?></a>. It has stopped receiving security updates. You should not use it for a live site.
				</p>
				<p>
					<?= $softwareName ?> will stop supporting your version of PHP in the near future. You should
					upgrade to a newer version of PHP at your earliest convenience. You can check <a href="https://www.akeeba.com/compatiblity.html">our Compatibility page</a> to see which versions of PHP are supported by each version of our software, select the newest one that fits your site's needs and upgrade your site to it. You can ask your host or your system administrator for instructions on upgrading PHP. It's easy and it will make your site faster and more secure.
				</p>
			</div>
		<?php elseif ($warn_about_maintenance): ?>
			<!-- Your PHP version has entered “Security Support” and will become EOL rather soon -->
			<div class="<?= $class_priority_low ?>">
				<h3>PHP <?= $shortVersion ?> is approaching End–of–Life</h3>

				<p>
					Your site is currently using PHP <?= $longVersion ?>. This version of PHP has entered its “Security maintenance” phase since <?= $securityDateFormatted ?> and has stopped receiving bug fixes. It will stop receiving security updates on <?= $eolDateFormatted ?> at which point it will be unsuitable for use on a live site. We recommend updating to a newer PHP version as soon as possible.
				</p>
			</div>
		<?php endif;
	}
...
if (!defined('KICKSTART'))
{
	try
	{
		akeeba_common_phpversion_warning(
			[
				// Configuration -- Override before calling this script
				'softwareName'           => $softwareName ?? 'This software',
				'class_priority_low'     => 'alert alert-info',
				'class_priority_medium'  => 'alert alert-warning',
				'class_priority_high'    => 'alert alert-danger',
				'warn_about_maintenance' => !isset($warn_about_maintenance) || $warn_about_maintenance,
				'eol_period_too_old'     => $eol_period_too_old ?? 'P3M',
				// Override these to test the script
				'longVersion'            => $longVersion ?? PHP_VERSION,
				'shortVersion'           => $shortVersion ?? sprintf('%d.%d', PHP_MAJOR_VERSION, PHP_MINOR_VERSION),
